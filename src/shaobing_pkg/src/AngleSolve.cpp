...
/**
 *  函数名: imu2ros_imu
 *  传入: Vector3d imu_pos            (视觉云台坐标)
 *  传出: Vector3d ros_imu_pos        (电控云台坐标)
 *  功能: 通过传入视觉云台坐标,转换为电控云台坐标
 */
Eigen::Vector3d AngleSolve::imu2ros_imu(Eigen::Vector3d imu_pos, double yaw)
{
    Eigen::Vector3d pos_tmp;
    Eigen::Vector3d ros_imu_pos;

    // 视觉系转电控系
    pos_tmp = {imu_pos[1], -imu_pos[0], imu_pos[2]};

    // 设置旋转矩阵和偏置向量(偏置向量在yaml文件中设置，目前为0,0,0)
    RotationMatrix_imu << 1, 0, 0,
        0, 1, 0,
        0, 0, 1;

    // 通过传入的欧拉角设置旋转矩阵
    RotationMatrix_imu = EulerAngle2RotationMatrix(0, 0, yaw); // 传入p,r,y角度制

    // 相机坐标系下加偏置
    pos_tmp += center_offset_position; // 原点偏移

    // 旋转
    ros_imu_pos = RotationMatrix_imu * pos_tmp; // 旋转矩阵
 ...
 *  函数名: pixel2ros_imu
 *  传入: Armor &armor                (装甲板结构体)
 *       double yaw                   (相机坐标系相较于云台坐标系的欧拉角)
 *  传出: Vector3d ros_imu_pos        (电控云台坐标)
 *  功能: 通过传入装甲板结构体和欧拉角,转换为电控云台坐标
 */
Eigen::Vector3d AngleSolve::pixel2ros_imu(Armor &armor, double yaw)
{
    armor.camera_position = pixel2cam(armor);
    Eigen::Vector3d imu_pos = cam2imu(armor.camera_position);
    Eigen::Vector3d ros_imu_pos = imu2ros_imu(imu_pos, yaw); // 电控云台坐标
 ...
 */
float AngleSolve::BulletModel(float x, float v, float angle)
{ // x:m,v:m/s,angle:rad
    float t, y;
...
double AngleSolve::Armor_Angle(Armor &armor)
{
    ...
    // 处理异常情况
    if (World_projection(armor, l) > World_projection(armor, -l))
        l = -l;
    yaw = l;
    return yaw;
}
