...
{
    ros::init(argc, argv, "camera3_node"); // argc和argv是main函数参数，camera3_node是节点名
    ...
    ros::Publisher pub = nh.advertise<new_msgs::vector_Armor>("camera3", 10); // 发送的话题名称为camera1。缓存长度为10
    // 循环
    while (ros::ok())
    {
        // 读取图片
